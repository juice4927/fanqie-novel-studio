...
const textWindows = (text: string, stride: number) => {
  ...
  const result: string[] = [];
  for (let index = 0; index + 24 <= normalized.length; index += stride)
    result.push(normalized.slice(index, index + 24));
  return result;
};
...
export class ResearchRepository {
  ...
  listBooks(): ResearchBook[] {
    return (
      this.db.prepare("SELECT payload FROM research_books ORDER BY updated_at DESC").all() as Array<{ payload: string }>
    ).map((row) => JSON.parse(row.payload) as ResearchBook);
  }
  ...
  getBook(bookId: string) {
    const row = this.db.prepare("SELECT payload FROM research_books WHERE id = ?").get(bookId) as
      | { payload: string }
      | undefined;
    if (!row) throw new Error("样本书不存在");
    const chapters = this.db
      .prepare("SELECT ordinal, title, content, word_count FROM research_chapters WHERE book_id = ? ORDER BY ordinal")
      .all(bookId) as Array<Record<string, unknown>>;
    return {
      book: JSON.parse(row.payload) as ResearchBook,
  ...
  listAnalyses(bookId: string): ResearchAnalysisRecord[] {
    return (
      this.db
        .prepare("SELECT payload FROM research_analyses WHERE book_id = ? ORDER BY from_chapter, layer")
        .all(bookId) as Array<{ payload: string }>
    ).map((row) => JSON.parse(row.payload) as ResearchAnalysisRecord);
  }

  updateBook(book: ResearchBook) {
    this.db
      .prepare("UPDATE research_books SET payload = ?, updated_at = ? WHERE id = ?")
      .run(JSON.stringify(book), now(), book.id);
  }

  listInsights(): InsightPack[] {
    return (
      this.db.prepare("SELECT payload FROM insights ORDER BY created_at DESC").all() as Array<{ payload: string }>
    ).map((row) => JSON.parse(row.payload) as InsightPack);
  }
  ...
  originalityMatches(content: string) {
    ...
    for (const window of textWindows(content, 1)) {
      ...
      for (const row of query.all(fingerprint) as Array<{ book_id: string }>) {
        matches.set(`${row.book_id}:${fingerprint}`, {
