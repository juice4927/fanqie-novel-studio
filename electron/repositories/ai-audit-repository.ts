...
function hydrateAiJob(row: Record<string, unknown>): AiJobRecord {
  return {
    ...
    status: String(row.status) as AiJobRecord["status"],
    inputTokens: Number(row.input_tokens ?? 0),
    outputTokens: Number(row.output_tokens ?? 0),
    actualCost: Number(row.actual_cost ?? 0),
    durationMs: Number(row.duration_ms ?? 0),
    headersAt: row.headers_at ? String(row.headers_at) : null,
    firstTokenAt: row.first_token_at ? String(row.first_token_at) : null,
...
export class AiAuditRepository {
  ...
  recoverInterrupted() {
    this.db
      .prepare(
        "UPDATE ai_jobs SET status = '已中断', error = '应用退出时任务仍在运行，可重新执行原操作', updated_at = ? WHERE status = '运行中'",
      )
      .run(now());
  }
  ...
  findSuccessful(taskType: string, inputHash: string, promptVersion: string, provider: string, model: string) {
    const row = this.db
      .prepare(
        "SELECT output FROM ai_jobs WHERE task_type = ? AND input_hash = ? AND prompt_version = ? AND provider = ? AND model = ? AND status = '成功' ORDER BY updated_at DESC LIMIT 1",
      )
  ...
  finish(id: string, output: string, error?: string, usage?: AiJobCompletion) {
    this.db
      .prepare(`UPDATE ai_jobs SET status = ?, output = ?, error = ?, input_tokens = ?, output_tokens = ?,
      ...
      .run(
        ...
        usage?.actualCost ?? 0,
        usage?.durationMs ?? 0,
        usage?.headersAt ?? null,
        usage?.firstTokenAt ?? null,
  ...
  markApplicationFailed(id: string, error: string) {
    this.db
      .prepare("UPDATE ai_jobs SET status = '失败', error = ?, updated_at = ? WHERE id = ? AND status = '成功'")
      .run(error, now(), id);
  }
  ...
  list(projectId?: string): AiJobRecord[] {
    const rows = (
      projectId
        ? this.db
            .prepare("SELECT * FROM ai_jobs WHERE project_id = ? ORDER BY created_at DESC LIMIT 200")
            .all(projectId)
        : this.db.prepare("SELECT * FROM ai_jobs ORDER BY created_at DESC LIMIT 200").all()
  ...
  retryContext(id: string) {
    const row = this.db.prepare("SELECT retry_context FROM ai_jobs WHERE id = ?").get(id) as
  ...
  prune(retentionDays = 90, maxRows = 5000) {
    ...
    const overflow = this.db
      .prepare(`DELETE FROM ai_jobs WHERE id IN (
