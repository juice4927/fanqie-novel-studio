...
export class BackgroundWorker {
  ...
  private pending = new Map<
    string,
    { resolve: (value: unknown) => void; reject: (reason: Error) => void; onProgress?: (value: unknown) => void }
  >();
  ...
  private spawn() {
    ...
    worker.on(
      "message",
      (message: { id: string; ok?: boolean; progress?: unknown; result?: unknown; error?: string }) => {
  ...
  run<T>(
    ...
    payload: unknown,
    options: { id?: string; onProgress?: (value: unknown) => void } = {},
  ): Promise<T> {
