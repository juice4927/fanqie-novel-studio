...
export class WorkspaceDatabase {
  ...
  constructor(root: string) {
    ...
    for (const directory of [root, this.projectsRoot, this.researchRoot, this.backupRoot]) mkdirSync(directory, { recursive: true });
  ...
  private initCatalog() {
    ...
    `);
    const projectColumns = this.catalog.prepare("PRAGMA table_info(projects)").all() as Array<{ name: string }>;
    if (!projectColumns.some((column) => column.name === "safe_stock_line")) {
      this.catalog.exec("ALTER TABLE projects ADD COLUMN safe_stock_line INTEGER NOT NULL DEFAULT 10");
    }
  }
  ...
  createProject(input: CreateProjectInput): ProjectSummary {
    ...
    this.catalog.prepare(`
      ...
      VALUES(?, ?, ?, '候选立项', ?, ?, ?, ?, ?)
    `).run(id, input.title.trim(), input.genre, input.targetWords, input.updateCadence, input.safeStockLine ?? 10, timestamp, timestamp);
  ...
  listProjects(): ProjectSummary[] {
    const rows = this.catalog.prepare("SELECT * FROM projects WHERE status != '归档' ORDER BY updated_at DESC").all() as unknown as CatalogRow[];
    return rows.map((row) => this.hydrateSummary(row));
  }

  getProjectSummary(id: string): ProjectSummary {
    const row = this.catalog.prepare("SELECT * FROM projects WHERE id = ?").get(id) as unknown as CatalogRow | undefined;
  ...
  private hydrateSummary(row: CatalogRow): ProjectSummary {
    ...
    const currentWords = chapters.reduce((sum, chapter) => sum + chapter.wordCount, 0);
    const stockChapters = chapters.filter((chapter) => chapter.status === "已定稿" || chapter.status === "待发布").length;
    const pendingHardIssues = issues.filter((issue) => issue.status === "待处理" && issue.severity === "硬性").length;
    const next = schedule
    ...
    return {
      ...
      riskLevel: pendingHardIssues > 0 ? "告警" : stockChapters < row.safe_stock_line && row.status === "连载中" ? "注意" : "正常",
  ...
  getProject(id: string): ProjectDetail {
    ...
    return {
      ...
      chapters: this.listRecords<Chapter>(db, "chapters").sort((a, b) => a.number - b.number),
  ...
  saveContract(id: string, contract: StoryContract) {
    ...
    const changed = JSON.stringify({
      premise: contract.premise, protagonistDesire: contract.protagonistDesire,
      readerPromise: contract.readerPromise, coreEmotion: contract.coreEmotion,
      ending: contract.ending, immutableRules: contract.immutableRules,
      prohibitedPatterns: contract.prohibitedPatterns,
    }) !== JSON.stringify({
      premise: previous.premise, protagonistDesire: previous.protagonistDesire,
      readerPromise: previous.readerPromise, coreEmotion: previous.coreEmotion,
      ending: previous.ending, immutableRules: previous.immutableRules,
    ...
    const next = { ...contract, approved: false, version: previous.version + 1, updatedAt: now() };
  ...
  approveContract(id: string) {
    ...
    if (!contract.premise || !contract.readerPromise || !contract.ending) throw new Error("创作契约的故事前提、读者承诺和终局不能为空");
  ...
  savePlan(id: string, plan: PlanNode) {
    ...
    const next = { ...plan, id: plan.id || randomUUID(), status: previous?.status ?? (plan.status === "待审批" ? "待审批" : "草稿") };
    const changed = previous && JSON.stringify({ ...next, status: undefined }) !== JSON.stringify({ ...previous, status: undefined });
    if (previous?.status === "已批准" && changed && !this.consumeApprovedChange(db, "规划", previous.id, this.entityVersion(db, "plans", previous.id))) {
  ...
  private persistChapter(id: string, chapter: Chapter, forcedStatus?: ChapterStatus) {
    const db = this.projectDb(id);
    const previous = chapter.id ? this.getRecord<Chapter>(db, "chapters", chapter.id) : undefined;
    const protectedContentChanged = previous && JSON.stringify({
      number: chapter.number, title: chapter.title, outline: chapter.outline, content: chapter.content,
      batchMode: chapter.batchMode, isKeyChapter: chapter.isKeyChapter,
    }) !== JSON.stringify({
      number: previous.number, title: previous.title, outline: previous.outline, content: previous.content,
      batchMode: previous.batchMode, isKeyChapter: previous.isKeyChapter,
    });
    const protectedEdit = Boolean(previous && ["已定稿", "待发布", "已发布"].includes(previous.status) && protectedContentChanged);
    ...
    const hasFactConflict = this.listRecords<LedgerFact>(db, "facts").some((fact) => fact.confidence === "有冲突");
    const hasHardIssue = this.listRecords<QualityIssue>(db, "issues").some((issue) => issue.chapterId === chapter.id && issue.severity === "硬性" && issue.status === "待处理");
    const isVolumeBoundary = volumeBoundaryChapters(this.listRecords<PlanNode>(db, "plans")).has(chapter.number);
    const contentChanged = Boolean(previous && previous.content !== chapter.content);
    let status: ChapterStatus = previous?.status ?? (chapter.content.trim() ? "草稿" : "章纲");
    if (forcedStatus) status = forcedStatus;
    else if (protectedEdit) status = chapter.content.trim() ? "待质检" : "章纲";
    else if (previous?.status === "章纲" && chapter.content.trim()) status = "草稿";
    else if (contentChanged && (previous?.status === "待质检" || previous?.status === "待定稿")) status = "草稿";
    const next: Chapter = {
      ...chapter,
      ...
      batchMode: chapter.isKeyChapter || hasFactConflict || hasHardIssue || isVolumeBoundary || majorStateChange.test(chapter.outline) ? "逐章" : chapter.batchMode,
      updatedAt: now(),
    };
    ...
    db.prepare("INSERT INTO chapter_fts_tri(id, title, content) VALUES(?, ?, ?)").run(next.id, next.title, next.content);
  ...
  transitionChapter(id: string, chapterId: string, status: ChapterStatus) {
    ...
    if (!transitions[chapter.status]?.includes(status)) throw new Error(`不允许从“${chapter.status}”直接变为“${status}”`);
    if ((status === "待定稿" || status === "已定稿" || status === "待发布") && issues.some((issue) => issue.chapterId === chapterId && issue.severity === "硬性" && issue.status !== "已解决")) {
  ...
  saveFact(id: string, fact: LedgerFact) {
    ...
    const conflicts = this.listRecords<LedgerFact>(db, "facts").filter((item) =>
      item.id !== next.id && item.subject === next.subject && item.predicate === next.predicate &&
      item.validToChapter === null && next.validToChapter === null && item.value !== next.value,
    );
    if (conflicts.length) next.confidence = "有冲突";
    this.saveRecord(db, "facts", next.id, next);
    this.saveEmbedding(db, "facts", next.id, `${next.kind} ${next.subject} ${next.predicate} ${next.value} ${next.knowledgeScope}`);
    this.touchProject(id);
    return next;
  }
  ...
  searchRelevantFacts(id: string, queryText: string, chapterNumber: number, limit = 80) {
    ...
    const rows = db.prepare("SELECT source_id, vector FROM embeddings WHERE source_type = 'facts'").all() as Array<{ source_id: string; vector: string }>;
    return rows
      .map((row) => ({ fact: facts.get(row.source_id), score: cosineSimilarity(query, parseJson<number[]>(row.vector)) }))
      .filter((item): item is { fact: LedgerFact; score: number } => Boolean(item.fact))
      .filter(({ fact }) => fact.validFromChapter <= chapterNumber && (fact.validToChapter === null || fact.validToChapter >= chapterNumber))
  ...
  searchProject(id: string, query: string): SearchHit[] {
    ...
    const db = this.projectDb(id);
    const chapters = new Map(this.listRecords<Chapter>(db, "chapters").map((chapter) => [chapter.id, chapter]));
    const hits = db.prepare("SELECT id, snippet(chapter_fts_tri, 2, '【', '】', '…', 24) AS excerpt FROM chapter_fts_tri WHERE chapter_fts_tri MATCH ? LIMIT 30").all(`"${normalized}"`) as Array<{ id: string; excerpt: string }>;
  ...
  listRevisions(id: string, collection: RevisionRecord["collection"], entityId: string): RevisionRecord[] {
    const db = this.projectDb(id);
    const rows = db.prepare("SELECT id, collection, entity_id, revision, payload, created_at FROM revisions WHERE collection = ? AND entity_id = ? ORDER BY revision DESC").all(collection, entityId) as Array<Record<string, unknown>>;
    return rows.map((row) => ({ id: String(row.id), collection: row.collection as RevisionRecord["collection"], entityId: String(row.entity_id), revision: Number(row.revision), payload: parseJson<unknown>(row.payload), createdAt: String(row.created_at) }));
  }

  restoreRevision(id: string, revisionId: string) {
    const db = this.projectDb(id);
    const row = db.prepare("SELECT collection, entity_id, payload FROM revisions WHERE id = ?").get(revisionId) as { collection: RevisionRecord["collection"]; entity_id: string; payload: string } | undefined;
    ...
    else if (row.collection === "state" && row.entity_id === "contract") this.saveContract(id, payload as StoryContract);
  ...
  saveIssues(id: string, chapterId: string, issues: QualityIssue[]) {
    ...
    for (const current of this.listRecords<QualityIssue>(db, "issues")) {
      if (current.chapterId === chapterId && current.status === "待处理") {
        this.saveRecord(db, "issues", current.id, { ...current, status: "已解决" });
      }
    }
    for (const issue of issues) this.saveRecord(db, "issues", issue.id, issue);
    if (issues.some((issue) => issue.severity === "硬性")) {
      const chapter = this.getRecord<Chapter>(db, "chapters", chapterId);
      if (chapter && chapter.batchMode !== "逐章") this.saveChapter(id, { ...chapter, batchMode: "逐章" });
    }
    this.touchProject(id);
  }
  ...
  resolveIssue(id: string, issueId: string, status: QualityIssue["status"]) {
    ...
    if (issue.severity === "硬性" && status === "已忽略") throw new Error("硬性质检项不能忽略，必须解决后才能继续");
  ...
  saveChangeRequest(id: string, change: ChangeRequest) {
    const db = this.projectDb(id);
    const baseVersion = this.targetVersion(db, change.targetKind, change.targetId);
    const next = { ...change, id: randomUUID(), baseVersion, createdAt: now(), status: "待审批" as const };
  ...
  decideChangeRequest(id: string, changeId: string, decision: "批准" | "拒绝") {
    ...
    this.saveRecord(db, "changes", changeId, { ...change, status: decision === "批准" ? "已批准" : "已拒绝" });
  }

  saveSchedule(id: string, item: ScheduleItem) {
    const db = this.projectDb(id);
    const project = this.getProjectSummary(id);
    const next = { ...item, id: item.id || randomUUID(), projectId: id, projectTitle: project.title };
  ...
  listRankings(): RankingSnapshot[] {
    const rows = this.research.prepare("SELECT * FROM ranking_snapshots ORDER BY captured_at DESC").all() as Array<Record<string, unknown>>;
    return rows.map((row) => ({
      id: String(row.id),
      ...
      entries: (this.research.prepare("SELECT * FROM ranking_entries WHERE snapshot_id = ? ORDER BY rank").all(String(row.id)) as Array<Record<string, unknown>>).map((entry) => ({
        id: String(entry.id), snapshotId: String(entry.snapshot_id), rank: Number(entry.rank), title: String(entry.title),
        author: String(entry.author), genre: String(entry.genre), words: Number(entry.words), status: String(entry.status),
        tags: parseJson<string[]>(entry.tags), sourceUrl: String(entry.source_url),
      })),
    }));
  }

  saveRanking(snapshot: RankingSnapshot) {
    this.research.prepare("INSERT INTO ranking_snapshots VALUES(?, ?, ?, ?, ?, ?)").run(
      snapshot.id, snapshot.source, snapshot.listName, snapshot.capturedAt, snapshot.status, snapshot.error,
    );
    const insert = this.research.prepare("INSERT INTO ranking_entries VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    for (const entry of snapshot.entries) insert.run(
      entry.id, snapshot.id, entry.rank, entry.title, entry.author, entry.genre, entry.words,
      entry.status, JSON.stringify(entry.tags), entry.sourceUrl,
    );
  }

  listResearchBooks(): ResearchBook[] {
    return (this.research.prepare("SELECT payload FROM research_books ORDER BY updated_at DESC").all() as Array<{ payload: string }>).map((row) => parseJson<ResearchBook>(row.payload));
  }

  saveResearchBook(book: ResearchBook, chapters: Array<{ title: string; content: string; wordCount: number }>) {
    this.research.prepare("INSERT OR REPLACE INTO research_books VALUES(?, ?, ?)").run(book.id, JSON.stringify(book), now());
    const insert = this.research.prepare("INSERT INTO research_chapters VALUES(?, ?, ?, ?, ?, ?)");
    const insertFingerprint = this.research.prepare("INSERT OR IGNORE INTO research_fingerprints VALUES(?, ?, ?)");
    chapters.forEach((chapter, index) => {
      insert.run(randomUUID(), book.id, index + 1, chapter.title, chapter.content, chapter.wordCount);
      for (const window of textWindows(chapter.content, 8)) insertFingerprint.run(hashText(window), book.id, window);
    });
  }

  getResearchBook(bookId: string) {
    const row = this.research.prepare("SELECT payload FROM research_books WHERE id = ?").get(bookId) as { payload: string } | undefined;
    if (!row) throw new Error("样本书不存在");
    const chapters = this.research.prepare("SELECT ordinal, title, content, word_count FROM research_chapters WHERE book_id = ? ORDER BY ordinal").all(bookId) as Array<Record<string, unknown>>;
    return {
      book: parseJson<ResearchBook>(row.payload),
      chapters: chapters.map((item) => ({ ordinal: Number(item.ordinal), title: String(item.title), content: String(item.content), wordCount: Number(item.word_count) })),
    };
  }

  saveResearchAnalyses(records: ResearchAnalysisRecord[]) {
    if (records[0]) this.research.prepare("DELETE FROM research_analyses WHERE book_id = ?").run(records[0].bookId);
    const insert = this.research.prepare("INSERT OR REPLACE INTO research_analyses VALUES(?, ?, ?, ?, ?, ?, ?)");
  ...
  listResearchAnalyses(bookId: string): ResearchAnalysisRecord[] {
    return (this.research.prepare("SELECT payload FROM research_analyses WHERE book_id = ? ORDER BY from_chapter, layer").all(bookId) as Array<{ payload: string }>).map((row) => parseJson<ResearchAnalysisRecord>(row.payload));
  }

  updateResearchBook(book: ResearchBook) {
    this.research.prepare("UPDATE research_books SET payload = ?, updated_at = ? WHERE id = ?").run(JSON.stringify(book), now(), book.id);
  }

  listInsights(): InsightPack[] {
    return (this.research.prepare("SELECT payload FROM insights ORDER BY created_at DESC").all() as Array<{ payload: string }>).map((row) => parseJson<InsightPack>(row.payload));
  }

  saveInsight(insight: InsightPack) {
    this.research.prepare("INSERT OR REPLACE INTO insights VALUES(?, ?, ?)").run(insight.id, JSON.stringify(insight), insight.createdAt);
  }
  ...
  findOriginalityMatches(content: string) {
    ...
    const query = this.research.prepare("SELECT book_id FROM research_fingerprints WHERE hash = ? LIMIT 5");
  ...
  findAiJob(taskType: string, inputHash: string, promptVersion: string, model: string) {
    const row = this.catalog.prepare(`
  ...
  startAiJob(projectId: string | null, taskType: string, inputHash: string, promptVersion: string, provider: string, model: string, inputSummary: string) {
    const id = randomUUID();
    const timestamp = now();
    this.catalog.prepare(`
    ...
    `).run(id, projectId, taskType, inputHash, promptVersion, provider, model, inputSummary, timestamp, timestamp);
    return id;
  }

  finishAiJob(id: string, output: string, error?: string) {
    this.catalog.prepare("UPDATE ai_jobs SET status = ?, output = ?, error = ?, updated_at = ? WHERE id = ?").run(
      error ? "失败" : "成功", error ? null : output, error ?? null, now(), id,
    );
  }
  ...
  getAiSettings(): AiSettings {
    ...
    return { baseUrl, model, embeddingModel, hasApiKey: false, inputPricePerMillion, outputPricePerMillion };
  }
  ...
  checkpointAll() {
    ...
    for (const db of this.projectDbs.values()) db.exec("PRAGMA wal_checkpoint(TRUNCATE)");
  }
  ...
  private getSetting(key: string, fallback: string) {
    const row = this.catalog.prepare("SELECT value FROM settings WHERE key = ?").get(key) as { value: string } | undefined;
  ...
  private consumeApprovedChange(db: DatabaseSync, targetKind: ChangeRequest["targetKind"], targetId: string, baseVersion: number) {
    const change = this.listRecords<ChangeRequest>(db, "changes")
      .filter((item) => item.status === "已批准" && item.targetKind === targetKind && item.targetId === targetId && item.baseVersion === baseVersion)
  ...
  private entityVersion(db: DatabaseSync, collection: RevisionRecord["collection"], entityId: string) {
    const row = db.prepare("SELECT MAX(revision) AS revision FROM revisions WHERE collection = ? AND entity_id = ?").get(collection, entityId) as { revision: number | null };
    return (row.revision ?? 0) + 1;
  }

  private targetVersion(db: DatabaseSync, targetKind: ChangeRequest["targetKind"], targetId: string) {
    if (targetKind === "创作契约") {
      if (targetId !== "contract") throw new Error("创作契约目标无效");
      return this.getState<StoryContract>(db, "contract").version;
    }
    if (targetKind === "规划") {
      if (!this.getRecord<PlanNode>(db, "plans", targetId)) throw new Error("变更目标规划不存在");
  ...
  private touchProject(id: string) {
    this.catalog.prepare("UPDATE projects SET updated_at = ? WHERE id = ?").run(now(), id);
  }
  ...
  private listRecords<T>(db: DatabaseSync, collection: string): T[] {
    return (db.prepare("SELECT payload FROM records WHERE collection = ?").all(collection) as Array<{ payload: string }>).map((row) => parseJson<T>(row.payload));
  }
  ...
  private saveRecord<T>(db: DatabaseSync, collection: string, id: string, payload: T, priorRevision?: number) {
    const previous = this.getRecord<T>(db, collection, id);
    if (previous) this.addRevision(db, collection, id, priorRevision ?? this.nextRevision(db, collection, id), previous);
  ...
  private saveEmbedding(db: DatabaseSync, sourceType: string, sourceId: string, text: string) {
    db.prepare("INSERT OR REPLACE INTO embeddings(id, source_type, source_id, content_hash, vector, updated_at) VALUES(?, ?, ?, ?, ?, ?)").run(
      `${sourceType}:${sourceId}`, sourceType, sourceId, hashText(text), JSON.stringify(localEmbedding(text)), now(),
    );
  }
  ...
  private updateSummaries(projectId: string, chapter: Chapter) {
    ...
    const finalized = project.chapters.filter((item) => ["已定稿", "待发布", "已发布"].includes(item.status));
    const saveSummary = (summary: Omit<StorySummary, "version" | "updatedAt">) => {
      const previous = this.getRecord<StorySummary>(db, "summaries", summary.id);
      this.saveRecord(db, "summaries", summary.id, { ...summary, version: (previous?.version ?? 0) + 1, updatedAt: now() }, previous?.version);
    };
    const scenes = chapter.content.split(/\n\s*\n/).map((item) => item.trim()).filter(Boolean).slice(0, 20);
    scenes.forEach((scene, index) => saveSummary({ id: `scene:${chapter.id}:${index + 1}`, layer: "场景", title: `第${chapter.number}章·场景${index + 1}`, fromChapter: chapter.number, toChapter: chapter.number, content: scene.slice(0, 220) }));
    saveSummary({ id: `chapter:${chapter.id}`, layer: "章节", title: `第${chapter.number}章 ${chapter.title}`, fromChapter: chapter.number, toChapter: chapter.number, content: `${chapter.outline}\n章末状态：${chapter.content.slice(-220)}`.trim() });
    const stageStart = Math.floor((chapter.number - 1) / 10) * 10 + 1;
    const stageEnd = stageStart + 9;
    const stageItems = this.listRecords<StorySummary>(db, "summaries").filter((item) => item.layer === "章节" && item.fromChapter >= stageStart && item.fromChapter <= stageEnd);
    saveSummary({ id: `stage:${stageStart}`, layer: "十章阶段", title: `第${stageStart}–${stageEnd}章`, fromChapter: stageStart, toChapter: Math.max(...stageItems.map((item) => item.toChapter), chapter.number), content: stageItems.map((item) => `${item.title}：${item.content}`).join("\n").slice(0, 6000) });
    const volumes = project.plans.filter((plan) => plan.kind === "分卷" && plan.status === "已批准").sort((a, b) => a.ordinal - b.ordinal);
    let cursor = 1;
    for (const volume of volumes) {
      ...
      if (chapter.number >= cursor && chapter.number <= end) {
        const items = this.listRecords<StorySummary>(db, "summaries").filter((item) => item.layer === "十章阶段" && item.fromChapter >= cursor && item.fromChapter <= end);
        saveSummary({ id: `volume:${volume.id}`, layer: "分卷", title: volume.title, fromChapter: cursor, toChapter: Math.min(end, Math.max(...finalized.map((item) => item.number))), content: `${volume.goal}\n${items.map((item) => item.content).join("\n")}`.slice(0, 10000) });
        break;
      }
      cursor = end + 1;
    }
    const stages = this.listRecords<StorySummary>(db, "summaries").filter((item) => item.layer === "十章阶段");
    saveSummary({ id: "book", layer: "全书", title: "全书进展摘要", fromChapter: 1, toChapter: Math.max(...finalized.map((item) => item.number)), content: stages.map((item) => `${item.title}：${item.content}`).join("\n").slice(-16000) });
  }
}
...
const majorStateChange = /死亡|牺牲|突破|晋级|身份揭露|身份暴露|决裂|成婚|离婚|重生|穿越|失忆|叛变|真相揭晓|终局/;
