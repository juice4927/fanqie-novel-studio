...
function splitChapters(text: string) {
  ...
  if (!matches.length)
    return [
      { title: "正文", content: normalized, wordCount: countWords(normalized) },
    ];
...
async function parseEpub(filePath: string) {
  ...
  const manifestItems = Array.isArray(packageData.manifest.item)
    ...
    : [packageData.manifest.item];
  const spineItems = Array.isArray(packageData.spine.itemref)
  ...
  for (const item of spineItems) {
    ...
    const title =
      cleanText($("h1, h2, title").first().text()) ||
...
export async function parseDocument(filePath: string): Promise<ImportPreview> {
  ...
  if (extension === ".txt") {
    ...
    if (result.messages.length)
      warnings.push(...result.messages.slice(0, 3).map((item) => item.message));
  ...
  if (chapters.length === 1)
    warnings.push("未识别到标准章节标题，请在导入后检查切章结果。");
  const duplicateTitles =
    chapters.length - new Set(chapters.map((chapter) => chapter.title)).size;
  if (duplicateTitles > 0)
    warnings.push(`发现 ${duplicateTitles} 个重复章节标题。`);
  return {
    fileName,
    sourceType,
    detectedEncoding,
...
export function qualityCheck(payload: QualityPayload): QualityIssue[] {
  ...
  if (chapter.wordCount < 1200)
    add(
      "警告",
      "篇幅",
      `本章仅 ${chapter.wordCount} 字，可能不足以完成情节目标`,
    );
  if (chapter.wordCount > 4200)
    add("建议", "篇幅", `本章 ${chapter.wordCount} 字，建议检查移动端阅读节奏`);
  if (chapter.content && !/[？?!！…]$/.test(chapter.content.trim()))
    add("建议", "章末钩子", "章末缺少明显的问题、变化或未完成动作");
  if (chapter.isKeyChapter && chapter.batchMode === "五章批次")
    add("硬性", "审批模式", "关键章节必须切换为逐章审批");
  for (const pattern of contract.prohibitedPatterns.filter(Boolean)) {
    if (chapter.content.includes(pattern))
      add("硬性", "禁写清单", `正文命中禁写项：${pattern}`, pattern);
  }
  ...
  if (
    chapter.outline &&
    !genreHints[genre].some((hint) => chapter.outline.includes(hint))
  ) {
    add(
      "建议",
      "题材规则",
      `章纲未明确体现${genre}常用的“${genreHints[genre].join(" / ")}”检查维度`,
    );
  }
  ...
  for (const fatigue of plugin.fatigueRules) {
    const matchesCurrent = fatigue.signals.some((signal) =>
      currentMechanism.includes(signal),
    );
    const recentMatches = recentChapters.filter((item) =>
      fatigue.signals.some((signal) =>
    ...
    if (matchesCurrent && recentMatches >= 2)
      add(
        ...
        fatigue.signals
          .filter((signal) => currentMechanism.includes(signal))
          .join("、"),
      );
  }
  return issues;
