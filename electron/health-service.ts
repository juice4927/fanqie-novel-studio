...
export async function scanSystemHealth(
  ...
  const projectRows = catalog.prepare("SELECT id, title FROM projects ORDER BY created_at").all() as Array<{
    id: string;
    title: string;
  }>;
  ...
  try {
    ...
    let failedAiJobs = Number(
      (catalog.prepare("SELECT COUNT(*) AS count FROM ai_jobs WHERE status = '失败'").get() as { count: number }).count,
    );
    ...
    checks.push({
      ...
      detail: failedAiJobs ? `共 ${failedAiJobs} 个失败任务，可在 AI 任务中心重试` : "没有失败的 AI 任务",
      projectId: null,
      repairable: false,
    });
    await advance("项目目录");
