...
export function SettingsPage({ api, notify }: { api: AppApi; notify: (message: string, tone?: "success" | "error") => void }) {
  const [settings, setSettings] = useState<AiSettings>({ baseUrl: "https://api.openai.com/v1", model: "gpt-4.1", embeddingModel: "text-embedding-3-small", hasApiKey: false, inputPricePerMillion: 0, outputPricePerMillion: 0, longTaskTimeoutMinutes: 10 });
  ...
  return <div className="page settings-page">
    <header className="page-header"><div><p className="eyebrow">本地工作台</p><h1>系统设置</h1><p>模型调用、凭据、独立工作区与加密备份。</p></div></header>
      <section className="settings-section"><div className="settings-title"><span className="settings-icon"><KeyRound size={19} /></span><div><h2>模型供应商</h2><p>GPT 模型自动使用 Responses API，其他模型使用兼容 Chat Completions 协议。</p></div><Badge tone={settings.hasApiKey ? "success" : "warning"}>{settings.hasApiKey ? "密钥已保存" : "未配置密钥"}</Badge></div><div className="settings-form"><Field label="API 基础地址"><Input value={settings.baseUrl} onChange={(event) => setSettings({ ...settings, baseUrl: event.target.value })} placeholder="https://api.openai.com/v1" /></Field><div className="form-grid two"><Field label="创作与分析模型"><Input value={settings.model} onChange={(event) => setSettings({ ...settings, model: event.target.value })} /></Field><Field label="相关事实检索" hint="使用本地汉字相似度索引，正文和账本不会因此上传"><Input value="本地检索（无需配置）" disabled /></Field></div><Field label="长任务总时限" hint="适用于语义质检、拆书、故事结构和批量章纲；流式连续 180 秒无数据仍会提前停止"><Select value={settings.longTaskTimeoutMinutes} onChange={(event) => setSettings({ ...settings, longTaskTimeoutMinutes: Number(event.target.value) })}><option value={5}>5 分钟</option><option value={10}>10 分钟（推荐）</option><option value={15}>15 分钟</option></Select></Field><div className="form-grid two"><Field label="输入价格 / 百万 Token"><Input type="number" min={0} step="0.01" value={settings.inputPricePerMillion} onChange={(event) => setSettings({ ...settings, inputPricePerMillion: Number(event.target.value) })} /></Field><Field label="输出价格 / 百万 Token"><Input type="number" min={0} step="0.01" value={settings.outputPricePerMillion} onChange={(event) => setSettings({ ...settings, outputPricePerMillion: Number(event.target.value) })} /></Field></div><Field label="API 密钥" hint={settings.hasApiKey ? "留空会保留 Windows Credential Manager 中的现有密钥" : "密钥保存到 Windows Credential Manager，不写入项目数据库或备份包"}><Input type="password" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder={settings.hasApiKey ? "已安全保存" : "输入 API 密钥"} /></Field><div className="settings-actions"><Button icon={<Save size={16} />} onClick={async () => { try { const saved = await api.saveAiSettings({ baseUrl: settings.baseUrl, model: settings.model, embeddingModel: settings.embeddingModel, inputPricePerMillion: settings.inputPricePerMillion, outputPricePerMillion: settings.outputPricePerMillion, longTaskTimeoutMinutes: settings.longTaskTimeoutMinutes }, apiKey || undefined); setSettings(saved); setApiKey(""); notify("模型设置已保存"); } catch (error) { notify(String(error), "error"); } }}>保存模型设置</Button></div></div></section>
