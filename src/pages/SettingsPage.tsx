...
import { Activity, ArchiveRestore, CalendarClock, Database, FolderLock, HardDrive, KeyRound, Play, RefreshCw, Save, ShieldCheck, Square, Wrench } from "lucide-react";
...
export function SettingsPage({ api, notify }: { api: AppApi; notify: (message: string, tone?: "success" | "error") => void }) {
  ...
  return <div className="page settings-page">
    ...
    <section className="settings-section">
      ...
      {aiJobs.length ? <div className="ai-job-list">{aiJobs.slice(0, 30).map((job) => <article key={job.id}><div><strong>{job.inputSummary}</strong><small>{job.taskType} · {job.model} · {new Date(job.createdAt).toLocaleString()}</small>{job.error && <small className="job-error">{job.error}</small>}</div><span className={`job-status job-${job.status}`}>{job.status}</span><span className="job-usage">{job.inputTokens + job.outputTokens} Token<br />¥{job.actualCost.toFixed(4)} · {(job.durationMs / 1000).toFixed(1)}s</span>{job.status === "运行中" && <Button variant="secondary" icon={<Square size={13} />} onClick={async () => { await api.cancelAiJob(job.id); setAiJobs(await api.listAiJobs()); }}>取消</Button>}</article>)}</div> : <p className="health-empty">还没有 AI 调用记录。失败或中断后可回到原操作重试，五章批次会跳过已有正文。</p>}
    </section>
