...
export function QualityPage({ project, api, reload, notify }: CommonProjectProps) {
  ...
  const recentConflictKeys = project.chapters
    .slice(-30)
    .map((chapter) => chapter.outline.replace(/[目标冲突结果：:；;，,。\s]/g, "").slice(0, 18))
    .filter(Boolean);
  const repeatedConflicts = recentConflictKeys.length - new Set(recentConflictKeys).size;
  const debts = [
    {
      label: "长期未回收伏笔",
    ...
    {
      label: "待兑现承诺",
      count: project.facts.filter((fact) => fact.kind === "承诺" && fact.validToChapter === null).length,
    },
    {
      label: "沉睡角色",
      count: project.facts.filter(
        (fact) => fact.kind === "人物" && fact.validToChapter === null && currentChapter - fact.evidenceChapter > 50,
      ).length,
    },
    {
      label: "能力膨胀信号",
      count: project.facts.filter((fact) => fact.kind === "能力" && currentChapter - fact.evidenceChapter <= 20).length,
    },
    { label: "重复冲突信号", count: repeatedConflicts },
    {
      label: "失联支线",
      count: project.facts.filter(
        (fact) => fact.kind === "支线" && fact.validToChapter === null && currentChapter - fact.evidenceChapter > 30,
      ).length,
    },
    {
      label: "冲突事实",
      count: project.facts.filter((fact) => fact.confidence === "有冲突").length,
    },
    {
      label: "未处理硬性项",
      count: project.issues.filter((issue) => issue.severity === "硬性" && issue.status === "待处理").length,
    },
  ];
  const reviewQueue = project.chapters
    .filter(
      (chapter) =>
        chapter.status === "待质检" ||
        project.issues.some((issue) => issue.chapterId === chapter.id && issue.status === "待处理"),
    )
    .map((chapter) => ({
      chapter,
  ...
  useEffect(() => {
    const ids = reviewQueueKey ? reviewQueueKey.split("|") : [];
    if (!ids.length) {
      setReviewChapterId("");
      return;
    }
    if (!ids.includes(reviewChapterId)) setReviewChapterId(ids[0]);
  ...
  return (
    <div className="page project-page quality-page">
      <header className="page-header quality-header">
        ...
        <Button icon={<GitPullRequestArrow size={16} />} variant="secondary" onClick={() => setChangeModal(true)}>
          提交改纲单
        </Button>
      </header>

      <section className="quality-summary" aria-label="质检概览">
        <div className={hardIssueCount ? "danger" : "clear"}>
          <AlertTriangle size={18} />
          <span>
            <strong>{hardIssueCount}</strong>硬性问题
          </span>
        </div>
        <div>
          <ClipboardCheck size={18} />
          <span>
            <strong>{reviewQueue.length}</strong>待审章节
          </span>
        </div>
        <div>
          <CircleDot size={18} />
          <span>
            <strong>{pendingIssues.length}</strong>待处理问题
          </span>
        </div>
        <div>
          <GitPullRequestArrow size={18} />
          <span>
            <strong>{project.changes.filter((item) => item.status === "待审批").length}</strong>待审批变更
          </span>
        </div>
      </section>
      ...
      <nav className="quality-tabs" aria-label="质检中心视图">
        <button
          type="button"
          className={qualityView === "审稿队列" ? "active" : ""}
          onClick={() => setQualityView("审稿队列")}
        >
          <ClipboardCheck size={16} />
          审稿队列<span>{reviewQueue.length}</span>
        </button>
        <button
          type="button"
          className={qualityView === "问题记录" ? "active" : ""}
          onClick={() => setQualityView("问题记录")}
        >
          <Archive size={16} />
          问题记录<span>{project.issues.length}</span>
        </button>
        <button
          type="button"
          className={qualityView === "改纲变更" ? "active" : ""}
          onClick={() => setQualityView("改纲变更")}
        >
      ...
      {qualityView === "审稿队列" &&
        (reviewQueue.length ? (
          <section className="quality-workbench">
            <aside className="quality-queue-panel">
              <header>
                ...
                <Badge tone={hardIssueCount ? "danger" : "neutral"}>{reviewQueue.length}</Badge>
              </header>
              <Select
                className="select quality-chapter-select"
                value={selectedReview?.chapter.id ?? ""}
                onChange={(event) => setReviewChapterId(event.target.value)}
                aria-label="筛选审稿章节"
              >
                {reviewQueue.map(({ chapter }) => (
                  <option key={chapter.id} value={chapter.id}>
                    第{chapter.number}章 · {chapter.title || "未命名章"}
                  </option>
                ))}
              </Select>
                ...
                {reviewQueue.map(({ chapter, issues }) => {
                  ...
                  return (
                    <button
                      type="button"
                      key={chapter.id}
                      className={selectedReview?.chapter.id === chapter.id ? "active" : ""}
                      onClick={() => setReviewChapterId(chapter.id)}
                    >
                      <span className="chapter-number">{chapter.number}</span>
                      <span className="chapter-label">
                        <strong>{chapter.title || "未命名章"}</strong>
                        <small>
                          {chapter.status} · {issues.length ? `${issues.length} 项待处理` : "等待质检"}
                        </small>
                      </span>
            ...
            <div className="quality-review-panel">
              <header className="quality-review-header">
                <div>
                  <p>
                    第 {selectedReview.chapter.number} 章 · {selectedReview.chapter.status}
                  </p>
                  <h2>{selectedReview.chapter.title || "未命名章"}</h2>
                  <span>
                    {selectedReview.chapter.wordCount.toLocaleString()} 字 · 修订 v{selectedReview.chapter.revision}
                  </span>
                </div>
                <div className="quality-review-actions">
                  <Button
                    variant="secondary"
                    disabled={Boolean(chapterAction)}
                    onClick={() => runChapterAction(selectedReview.chapter, "质检")}
                  >
                    {chapterAction?.kind === "质检" ? (
                      <>
                  ...
                  {selectedReview.issues.length > 0 &&
                    !["已定稿", "待发布", "已发布"].includes(selectedReview.chapter.status) && (
                      <Button
                        disabled={Boolean(chapterAction)}
                        onClick={() => runChapterAction(selectedReview.chapter, "修订")}
                      >
                        {chapterAction?.kind === "修订" ? (
                          <>
                            <LoaderCircle className="spin" size={15} />
                            修订中
                          </>
              ...
              <div className="issue-toolbar">
                <div>
                  <Segmented options={["待处理", "全部"] as const} value={issueScope} onChange={setIssueScope} />
                </div>
                <Select
                  value={issueSeverity}
          ...
          <EmptyState
            ...
            title="审稿队列已清空"
            description="当前没有等待质检或问题处理的章节。"
          />
        ))}

      {qualityView === "问题记录" && (
        <section className="section-band quality-records">
          <div className="section-heading">
            <div>
              <h2>问题记录</h2>
              <p>查看跨章节问题与处理结果</p>
            </div>
            <div className="issue-record-filters">
              <Segmented options={["待处理", "全部"] as const} value={issueScope} onChange={setIssueScope} />
              <Select
                value={issueSeverity}
                onChange={(event) => setIssueSeverity(event.target.value as typeof issueSeverity)}
      ...
      {qualityView === "改纲变更" && (
        <section className="section-band quality-changes">
          <div className="section-heading">
            <div>
              <h2>改纲变更单</h2>
              <p>批准只改变状态，不会静默改写内容。</p>
            </div>
            <Button icon={<Plus size={15} />} onClick={() => setChangeModal(true)}>
              提交改纲单
            </Button>
          </div>
            ...
            <div className="change-list">
              {project.changes.map((item) => (
                <article key={item.id}>
                  <div>
                    <Badge
                      tone={item.status === "已批准" ? "success" : item.status === "已拒绝" ? "danger" : "warning"}
                    >
                      {item.status}
                    </Badge>
                    <h3>{item.title}</h3>
      ...
      <details className="quality-debts">
        <summary>
          ...
          <small>{debts.reduce((sum, debt) => sum + debt.count, 0)} 个信号</small>
        </summary>
      ...
      {changeModal && (
        <Modal title="提交改纲变更单" onClose={() => setChangeModal(false)}>
          <div className="form-stack">
            <div className="form-grid two">
              <Field label="目标类型">
                <Select
                  ...
                  onChange={(event) => {
                    const targetKind = event.target.value as ChangeRequest["targetKind"];
                    setChange({
                      ...change,
                      targetKind,
              ...
              <Field label="具体目标">
                <Select
                  ...
                  onChange={(event) => setChange({ ...change, targetId: event.target.value })}
                >
            ...
            <Field label="变更标题">
              <Input value={change.title} onChange={(event) => setChange({ ...change, title: event.target.value })} />
            </Field>
            <Field label="变更原因">
              <Textarea
                rows={3}
            ...
            <div className="form-grid two">
              <Field label="旧方案">
                <Textarea
                  ...
                  onChange={(event) => setChange({ ...change, beforeValue: event.target.value })}
                />
              </Field>
              <Field label="新方案">
                <Textarea
                  ...
                  onChange={(event) => setChange({ ...change, afterValue: event.target.value })}
                />
              </Field>
            </div>
            <Field label="影响范围">
              <Textarea
                rows={3}
                value={change.impact}
                onChange={(event) => setChange({ ...change, impact: event.target.value })}
              />
            </Field>
            <Field label="回滚方式">
              <Textarea
                ...
                onChange={(event) => setChange({ ...change, rollback: event.target.value })}
              />
            </Field>
  ...
  if (!issues.length) {
    return (
      <div className="quality-empty">
        ...
        <span>调整筛选条件，或从左侧选择其他章节。</span>
      </div>
    );
  }
  return (
    <div className="issue-list quality-issue-list">
      {issues.map((issue) => {
        const chapter = showChapter ? chapters.find((item) => item.id === issue.chapterId) : undefined;
        return (
          <article key={issue.id} className={issue.status !== "待处理" ? "resolved" : ""}>
            <div className="issue-marker">
              {issue.status === "待处理" ? <AlertTriangle size={17} /> : <Check size={17} />}
            </div>
            ...
            {issue.status === "待处理" ? (
              <div className="issue-actions">
                {issue.severity !== "硬性" && (
                  <Button variant="ghost" onClick={() => onUpdate(issue.id, "已忽略")}>
                    忽略
                  </Button>
                )}
                <Button variant="secondary" icon={<Check size={14} />} onClick={() => onUpdate(issue.id, "已解决")}>
                  解决
                </Button>
              </div>
            ) : (
