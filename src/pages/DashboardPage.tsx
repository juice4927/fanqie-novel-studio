import { AlertTriangle, ArrowRight, BookOpen, CalendarClock, FileCheck2, LibraryBig, Plus, ShieldAlert, Trash2 } from "lucide-react";
...
export function DashboardPage({ data, onCreate, onOpenProject, onDeleteProject }: { data: DashboardData; onCreate: () => void; onOpenProject: (id: string) => void; onDeleteProject: (project: ProjectSummary) => void }) {
  return <div className="page">
    ...
    <section className="section-band">
      ...
      {data.projects.length ? <div className="project-table">
        ...
        {data.projects.map((project) => <div className="project-row" key={project.id} role="button" tabIndex={0} onClick={() => onOpenProject(project.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpenProject(project.id); }}>
          ...
          <span className="project-row-actions"><IconButton label={`删除作品 ${project.title}`} onClick={(event) => { event.stopPropagation(); onDeleteProject(project); }}><Trash2 size={16} /></IconButton><ArrowRight size={17} /></span>
        </div>)}
