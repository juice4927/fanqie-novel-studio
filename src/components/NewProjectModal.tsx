...
export function NewProjectModal({
  ...
  const [input, setInput] = useState<BookConceptInput>({
    genre: GENRES[0],
    targetWords: 1000000,
  ...
  return (
    <Modal title="从 0 开始创建一本书" onClose={() => !busy && onClose()} width={920}>
      <div className="book-wizard">
        ...
        <div className="form-grid three">
          <Field label="平台主题材">
            <Select
              value={input.genre}
              onChange={(event) => {
                setInput({ ...input, genre: event.target.value as Genre });
                setConcepts([]);
              }}
              disabled={busy}
            >
          ...
          <Field label="目标字数">
            <Input
              ...
              onChange={(event) => setInput({ ...input, targetWords: Number(event.target.value) })}
              disabled={busy}
            />
          </Field>
          <Field label="更新节奏">
            <Input
              value={input.updateCadence}
              onChange={(event) => setInput({ ...input, updateCadence: event.target.value })}
              disabled={busy}
            />
          </Field>
        </div>
        <div className="genre-composer">
            ...
            <div className="genre-option-grid">
              {NARRATIVE_GENRES.map((genre) => (
                <label key={genre} className="check-row">
                  <input
                    type="checkbox"
                    checked={input.secondaryGenres?.includes(genre) ?? false}
                    disabled={
                      busy || (!input.secondaryGenres?.includes(genre) && (input.secondaryGenres?.length ?? 0) >= 3)
                    }
                    onChange={() => {
                      const current = input.secondaryGenres ?? [];
                      const next = current.includes(genre)
                        ? current.filter((item) => item !== genre)
                        : ([...current, genre] as NarrativeGenre[]);
                      setInput({ ...input, secondaryGenres: next });
                      setConcepts([]);
                    }}
                  />
                  {genre}
              ...
              <div className="genre-option-grid">
                {group.elements.map((element) => (
                  <label key={element} className="check-row">
                    <input
                      type="checkbox"
                      checked={input.genreElements?.includes(element) ?? false}
                      disabled={
                        busy || (!input.genreElements?.includes(element) && (input.genreElements?.length ?? 0) >= 8)
                      }
                      onChange={() => {
                        const current = input.genreElements ?? [];
                        const next = current.includes(element)
                          ? current.filter((item) => item !== element)
                          : [...current, element];
                        setInput({ ...input, genreElements: next });
                        setConcepts([]);
                      }}
                    />
                    {element}
          ...
          <Field label="自定义创作方向" hint="可补充列表里没有的混合方式、反套路要求或题材边界">
            <Input
              value={input.customGenreDirection ?? ""}
              onChange={(event) => {
                setInput({ ...input, customGenreDirection: event.target.value });
                setConcepts([]);
              }}
        ...
        {mode === "手动创建" ? (
          ...
          <>
            <Field
              label="你已有的灵感（可不填）"
              hint={`系统以 ${input.genre} 为商业基线，再结合上面的复合类型与元素；可只写一句人物、情境或想要的情绪。`}
            >
              <Textarea
                value={input.seed}
                onChange={(event) => setInput({ ...input, seed: event.target.value })}
                placeholder="例如：女主重回八零年代，不想再替妹妹牺牲；留空则完全由 AI 提案。"
                disabled={busy}
              />
            </Field>
            <div className="wizard-genre-note">
              <strong>{selectedDirection ? "本次题材组合" : "平台商业基线"}</strong>
              <span>
                {selectedDirection || `${plugin.readerPromise}；未指定复合方向时，三套方案会分别选择不同叙事主轴。`}
              </span>
            </div>
            {!concepts.length ? (
              <div className="wizard-generate">
                ...
                <div>
                  ...
                  <span>每套包含书名、故事前提、核心卖点、目标读者、长篇发动机和未审批创作契约。</span>
                </div>
                <Button
              ...
              <div className="book-concept-grid">
                {concepts.map((concept) => (
                  ...
                  >
                    ...
                    <dl>
                      <dt>叙事主轴</dt>
                      <dd>{concept.secondaryGenres.join(" + ")}</dd>
                      <dt>开局机制</dt>
                      <dd>{concept.openingMechanism}</dd>
                      <dt>成长载体</dt>
                      <dd>{concept.growthCarrier}</dd>
                      <dt>主要回报</dt>
                      <dd>{concept.primaryPayoff}</dd>
                      <dt>商业钩子</dt>
                      <dd>{concept.commercialHook}</dd>
                      <dt>目标读者</dt>
                      <dd>{concept.audience}</dd>
                      <dt>长篇发动机</dt>
                      <dd>{concept.longFormEngine}</dd>
                      <dt>故事终局</dt>
                      <dd>{concept.ending}</dd>
                      <dt>明确避用</dt>
                      <dd>{concept.prohibitedPatterns.join("；")}</dd>
                    </dl>
                  </button>
                ))}
              </div>
        ...
        <div className="modal-actions">
          ...
          <Button disabled={busy || (mode === "手动创建" ? !manualTitle.trim() : !selected)} onClick={create}>
