...
type ChapterBatchProject = Pick<
  ProjectDetail,
  "summary" | "contract" | "plans" | "chapters" | "facts" | "issues"
>;

type ChapterBatchPricing = Pick<
  AiSettings,
...
export function buildChapterBatchPreview(
  ...
  if (start.batchMode !== "五章批次")
    return blocked("请先保存“五章批次”模式");
  if (!project.contract.approved)
    return blocked("创作契约审批后才能生成正文");
  ...
  for (const chapter of ordered) {
    if (chapter.batchMode !== "五章批次")
      return blocked(
        `第${chapter.number}章处于逐章模式`,
        chapterSummaries,
      );
    if (chapter.isKeyChapter)
      return blocked(
        `第${chapter.number}章是关键章，必须逐章审批`,
        chapterSummaries,
      );
    ...
    if (activeHardIssues.has(chapter.id))
      return blocked(
        `第${chapter.number}章有未解决的硬性告警`,
        chapterSummaries,
      );
    if (!chapter.outline.trim())
      return blocked(
        `第${chapter.number}章尚未填写章纲`,
        chapterSummaries,
      );
    if (["已定稿", "待发布", "已发布"].includes(chapter.status))
      return blocked(
        `第${chapter.number}章已经定稿或进入发布流程`,
        chapterSummaries,
      );
    const hardConstraint = evaluateStoryConstraints(
      project.facts,
      chapter,
    ).find((finding) => finding.severity === "硬性");
    if (hardConstraint) {
      return blocked(
        `第${chapter.number}章写前约束失败：${hardConstraint.message}`,
        chapterSummaries,
      );
    }
  }
  ...
  const inputTokens = ordered.reduce(
    (sum, chapter) => sum + estimateInputTokens(chapter),
    0,
  );
