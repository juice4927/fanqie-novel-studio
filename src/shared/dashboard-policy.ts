...
export function deriveProjectRisk(input: {
  ...
  if (
    input.status === "连载中" &&
    input.stockChapters < input.safeStockLine
  ) {
    return "注意";
  }
  return "正常";
...
export function compareDashboardProjects(
  left: ProjectSummary,
  right: ProjectSummary,
): number {
  return (
    compareTimestamps(left.updatedAt, right.updatedAt, -1) ||
    left.id.localeCompare(right.id)
  );
}

export function compareDueSchedules(
  left: ScheduleItem,
  right: ScheduleItem,
): number {
  return (
    compareTimestamps(left.publishAt, right.publishAt, 1) ||
    left.id.localeCompare(right.id)
  );
}

export function compareActiveAlerts(
  left: QualityIssue,
  right: QualityIssue,
): number {
  return (
    compareTimestamps(left.createdAt, right.createdAt, -1) ||
    left.id.localeCompare(right.id)
  );
}

export function visibleProjectSummaries(
  projects: ReadonlyArray<ProjectSummary>,
): ProjectSummary[] {
  return projects
    .filter((project) => isVisibleProjectStatus(project.status))
...
export function collectDashboardActivity(
  ...
  const visible = projects.filter((project) =>
    isVisibleProjectStatus(project.summary.status),
  );
  ...
  return {
    dueToday: visible
      ...
      .filter(
        (item) =>
          item.status !== "已发布" &&
          Date.parse(item.publishAt) < cutoff,
      )
...
export function assembleDashboard(
  ...
  return {
    ...
    activeAlerts: [...activity.activeAlerts]
      .sort(compareActiveAlerts)
      .slice(0, 12),
    totals: {
      activeBooks: projects.filter((project) =>
