...
export function analyzeMetrics(metrics: MetricSnapshot[]): ReviewSuggestion[] {
    ...
    return [
      {
        id: "data-volume",
        category: "数据质量",
        observation: "当前指标不足两个时间点。",
        recommendation: "继续收集至少一周数据后再判断趋势，不据单点数据改纲。",
        evidence: `${ordered.length} 个时间点`,
        confidence: "低",
      },
    ];
  ...
  if (funnel.clickRate !== null && funnel.clickRate < 10)
    suggestions.push({
      id: "click-rate",
      category: "简介",
      observation: "曝光到点击的入口转化偏低。",
      recommendation: "先核对书名、封面和简介是否向同一目标读者承诺同一种核心幻想，不改正文主线。",
      evidence: `点击率 ${funnel.clickRate.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  if (funnel.firstChapterCompletion !== null && funnel.firstChapterCompletion < 35)
    suggestions.push({
      id: "chapter-one",
      category: "开篇",
      observation: "首章读完率偏低。",
      recommendation: "复核首章是否尽早落下主角目标、具体压力、主动行动和首次可见反馈。",
      evidence: `首章读完率 ${funnel.firstChapterCompletion.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  if (funnel.threeChapterRetention !== null && funnel.threeChapterRetention < 25)
    suggestions.push({
      id: "three-chapter",
      category: "转化",
      observation: "三章留存偏低。",
      recommendation: "检查前三章是否完成一次完整承诺与回报循环，并建立清晰的下一阶段目标。",
      evidence: `三章留存 ${funnel.threeChapterRetention.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  if (funnel.bookshelfRate !== null && funnel.bookshelfRate < 5)
    suggestions.push({
      id: "bookshelf",
      category: "转化",
      observation: "阅读到加书架的转化偏低。",
      recommendation: "检查当前免费阅读段是否形成稳定的长线承诺，而不是只完成一次性反转。",
      evidence: `加书架率 ${funnel.bookshelfRate.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  ...
  if (retentionChange <= -3) {
    ...
    suggestions.push({
      id: "retention-drop",
      category: "节奏",
      observation: "后半段平均留存较前半段下降。",
      recommendation: `复核${weakest?.chapterNumber ? `第${weakest.chapterNumber}章附近` : "下降时间点附近"}的目标兑现、信息增量和章末推动力，先提交近期章纲变更单。`,
      evidence: `留存变化 ${retentionChange.toFixed(1)} 个百分点`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  }
  if (readChange < -15 && average(after, "exposure") >= average(before, "exposure") * 0.9)
    suggestions.push({
      id: "read-drop",
      category: "开篇",
      observation: "曝光相对稳定但阅读下降。",
      recommendation: "检查书名、简介和最近发布章节的入口承诺是否一致，不直接归因于单章。",
      evidence: `阅读变化 ${readChange.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  if (followRateAfter + 0.5 < followRateBefore)
    suggestions.push({
      id: "follow-drop",
      category: "章纲",
      observation: "阅读到追读的转化走低。",
      recommendation: "检查未来五章是否连续兑现核心承诺，并减少重复冲突。",
      evidence: `追读率 ${followRateBefore.toFixed(1)}% → ${followRateAfter.toFixed(1)}%`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  if (!suggestions.length)
    suggestions.push({
      id: "stable",
      category: "数据质量",
      observation: "当前数据未出现达到规则阈值的明显变化。",
      recommendation: "保持现有规划，继续按周记录；不要因小幅波动改动终局或卷目标。",
      evidence: `${ordered.length} 个时间点`,
      confidence: ordered.length >= 7 ? "中" : "低",
    });
  return suggestions;
