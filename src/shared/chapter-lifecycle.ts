...
export function isProtectedChapterEdit(previous: Chapter | undefined, next: Chapter) {
  return Boolean(
    previous && PROTECTED_CHAPTER_STATUSES.includes(previous.status) && chapterContentChanged(previous, next),
  );
}
...
export function deriveChapterBatchMode(chapter: Chapter, context: ChapterBatchModeContext): Chapter["batchMode"] {
  const hasFactConflict = context.facts.some((fact) => fact.confidence === "有冲突");
  ...
  return chapter.isKeyChapter ||
    ...
    hasHardIssue ||
    isVolumeBoundary ||
    hasMajorStateChange(chapter.outline, context.genre, context.majorStateChanges)
    ? "逐章"
    : chapter.batchMode;
}
...
export function prepareChapterSave(chapter: Chapter, options: PrepareChapterSaveOptions): Chapter {
  return {
    ...chapter,
    ...
    revision: options.previous ? options.previous.revision + (options.createRevision ? 1 : 0) : 1,
