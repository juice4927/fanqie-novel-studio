...
function repeatedRatio(text: string) {
  ...
  return [...windows.values()].filter((count) => count > 1).reduce((sum, count) => sum + count - 1, 0) / Math.max(windows.size, 1);
}

export function evaluatePromptOutput(fixture: PromptFixture, output: unknown, inputTokens = 0, outputTokens = 0): PromptEvaluation {
...
export function comparePromptVersions(baseline: PromptEvaluation[], candidate: PromptEvaluation[]) {
  const average = (items: PromptEvaluation[]) => items.reduce((sum, item) => sum + item.totalScore, 0) / Math.max(items.length, 1);
  const baselineScore = average(baseline);
  const candidateScore = average(candidate);
  return { baselineScore, candidateScore, delta: candidateScore - baselineScore, regressed: candidateScore + 2 < baselineScore };
}
