...
const chapter = {
  id: "chapter-2", number: 2, title: "旧标题", outline: "旧章纲", content: "", wordCount: 0,
  status: "章纲", batchMode: "逐章", isKeyChapter: false, revision: 7, updatedAt: "2026-07-31T00:00:00.000Z",
} satisfies Chapter;
...
describe("chapter AI retry validation", () => {
  it("serializes and validates chapter and former batch retry contexts", () => {
    ...
    expect(chapterContext).toMatchObject({
      kind: "chapter", projectId: "project-1", chapterId: "chapter-2", revision: 7,
  ...
  it("rejects non-retryable and unrelated AI jobs", () => {
    ...
    expect(() => validateChapterAiRetrySource(sourceJob({ taskType: "quality-review" }), raw)).toThrow("只支持重试章节正文");
  ...
  it("rejects damaged, legacy, and structurally invalid contexts", () => {
    ...
    expect(() => validateChapterAiRetrySource(sourceJob(), JSON.stringify({
      kind: "chapter", projectId: "project-1", chapterId: "chapter-2",
    }))).toThrow("旧版或无效");
    expect(() => validateChapterAiRetrySource(sourceJob(), JSON.stringify({
      kind: "unknown", projectId: "project-1", chapterId: "chapter-2", revision: 7,
    }))).toThrow("旧版或无效");
    expect(() => validateChapterAiRetrySource(sourceJob(), JSON.stringify({
      kind: "chapter", projectId: "project-1", chapterId: "chapter-2", revision: 7.5,
    }))).toThrow("旧版或无效");
  });
  ...
  it("blocks a retry after the chapter revision or target changes", () => {
    ...
    expect(() => assertChapterRetrySnapshot(context, { ...chapter, outline: "自动保存后的新章纲" })).toThrow("已被修改");
