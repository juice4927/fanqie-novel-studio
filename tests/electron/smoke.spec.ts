  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      await api.saveContract(project.id, {
        ...
        keyRelationships: [
          "维修员与安全主管从互相防备到共同核验证据",
          "维修员与企业负责人围绕事故责任和家人安全持续对抗",
        ],
        worldRules: [
          "事故回声只能提示尚未发生的危险片段，不能直接给出原因",
          "每次追查都必须由维修记录和现场证据交叉验证",
        ],
      ...
      for (let number = 1; number <= 5; number += 1) {
        await api.saveChapter(project.id, {
          ...
          number,
          title: `批次第${number}章`,
          outline: `目标：完成第${number}步；冲突：资源有限；结果：线索推进`,
          content: "",
          wordCount: 0,
          status: "章纲",
          batchMode: "五章批次",
      ...
      const plan = await api.savePlan(project.id, {
        id: "",
        kind: "细纲",
        title: "旧章纲",
        ordinal: 1,
        ...
        chapters: [
          {
            ...
            after: {
              title: "残缺记录",
              outline: "先取得记录，再核对时间矛盾",
              chapterFunction: "调查",
              targetWords: 2200,
              chapterPromise: "验证事故线索",
              expectedPayoff: "确认一处矛盾",
              crisis: "记录即将被销毁",
              endingExpectation: "矛盾指向维修主管",
              expectationTargetChapter: 3,
            },
          },
        ],
      });
      ...
      try {
        await api.applyPlanningRepairs(project.id, {
          plans: [
            {
              targetId: plan.id,
              after: {
      ...
      return {
        workspace: await api.getWorkspacePath(),
        projectId: project.id,
        title: detail.summary.title,
        firstOverviewContent: first.content,
        loadedFirstId: loadedFirst.id,
        batch,
        autoBackup,
        health,
        repaired,
        protectedRepairBlocked,
      };
    });
    ...
    expect(result.repaired).toMatchObject({
      appliedPlanIds: [expect.any(String)],
      appliedChapterIds: [expect.any(String)],
    });
    ...
    await expect(page.getByRole("heading", { name: "多书总览" })).toBeVisible();
    const geolocationPermission = await page.evaluate(
