...
function tempRoot() {
  const root = mkdtempSync(path.join(os.tmpdir(), "novel-import-test-"));
  roots.push(root);
  return root;
}
...
describe("research document importers", () => {
  ...
  it("previews DOCX chapters", async () => {
    ...
    const document = new Document({
      sections: [
        {
          children: [
            new Paragraph("第1章 开端"),
            new Paragraph("文档正文".repeat(20)),
            new Paragraph("第2章 继续"),
            new Paragraph("后续正文".repeat(20)),
          ],
        },
      ],
    });
  ...
  it("previews EPUB content in spine order", async () => {
    ...
    zip.file(
      "OEBPS/content.opf",
      `<package><manifest><item id="c1" href="c1.xhtml"/><item id="c2" href="c2.xhtml"/></manifest><spine><itemref idref="c1"/><itemref idref="c2"/></spine></package>`,
    );
    zip.file(
      "OEBPS/c1.xhtml",
      `<html><head><title>第一章</title></head><body><h1>第一章</h1><p>${"正文一".repeat(20)}</p></body></html>`,
    );
    zip.file(
      "OEBPS/c2.xhtml",
      `<html><head><title>第二章</title></head><body><h1>第二章</h1><p>${"正文二".repeat(20)}</p></body></html>`,
    );
