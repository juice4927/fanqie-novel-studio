...
import type { ChangeRequest, Chapter, LedgerFact, PlanNode, QualityIssue } from "../src/shared/types";
...
describe("shared chapter lifecycle", () => {
  it("derives editing status without accepting status injection", () => {
    ...
    expect(deriveChapterStatus(chapter("已定稿"), chapter("已定稿", "修改正文"), { protectedEdit: true })).toBe(
      "待质检",
    );
  ...
  it("derives one-chapter mode from every project-level gate", () => {
    ...
    expect(
      deriveChapterBatchMode(candidate, {
        ...context,
        facts: [
          {
            id: "fact-1",
