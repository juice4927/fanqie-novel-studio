...
import {
  ...
  decodeFanqiePublicText,
  fetchPublicHtml,
  parseFanqieDetailPage,
} from "../electron/ranking-service";
...
describe("ranking analytics", () => {
  ...
  it("builds market opportunities only from repeated snapshots of the same list", () => {
    const makeEntry = (snapshotId: string, rank: number, title: string) => ({
      id: `${snapshotId}-${title}`,
      snapshotId,
      rank,
      title,
    ...
    const snapshots: RankingSnapshot[] = [
      {
        ...
        status: "成功",
        error: null,
        entries: [makeEntry("old", 3, "甲"), makeEntry("old", 2, "乙")],
      },
      {
        id: "other",
        source: "番茄小说官网",
        ...
        status: "成功",
        error: null,
        entries: [makeEntry("other", 1, "无关作品")],
      },
      {
        id: "new",
        source: "番茄小说官网",
        ...
        status: "成功",
        error: null,
        entries: [makeEntry("new", 1, "甲"), makeEntry("new", 2, "丙")],
      },
    ];
  ...
  it("times out stalled public page requests", async () => {
    ...
    await expect(fetchPublicHtml(new URL("https://example.com/rank"), { timeoutMs: 5, attempts: 1 })).rejects.toThrow(
      "公开页请求超时",
    );
  });
  ...
  it("rejects local and private destinations before fetching", async () => {
    ...
    await expect(fetchPublicHtml(new URL("http://127.0.0.1/private"), { attempts: 1 })).rejects.toThrow(
      /私有网络|本机/,
    );
    await expect(fetchPublicHtml(new URL("http://169.254.169.254/latest/meta-data"), { attempts: 1 })).rejects.toThrow(
      /私有网络|保留地址/,
    );
  ...
  it("revalidates every redirect target", async () => {
    ...
    await expect(fetchPublicHtml(new URL("http://93.184.216.34/rank"), { attempts: 1 })).rejects.toThrow(
      /私有网络|本机/,
    );
  });
  ...
  it("captures only public page metadata", async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(
            `<html><body><article class="book-item"><span class="rank">1</span><a href="/page/123" title="公开书名">公开书名</a><span class="author">公开作者</span><span>都市脑洞 88万字 连载</span></article></body></html>`,
    ...
    vi.stubGlobal(
      ...
      vi.fn(
        async (input: string | URL | Request) =>
          new Response(String(input).includes("/page/123") ? detailHtml : rankHtml, {
            status: 200,
  ...
  it("reads exactly ten free public opening chapters and decodes reader text", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        const state = {
          reader: { chapterData: { needPay: 0, order: String(order), title: `第${order}章 标题${order}` } },
        };
        return new Response(
          `<div class="muye-reader-content"><div><p></p><p>公开正文${order}</p></div></div><script>window.__INITIAL_STATE__=${JSON.stringify(state)};</script>`,
          { status: 200 },
        );
      }),
    );
...
describe("metric review", () => {
  ...
  it("parses legacy and expanded CSV headers without shifting columns", () => {
    let sequence = 0;
    const legacy = parseMetricsCsv(
      "日期,章节,曝光,阅读,留存,追读,收益,评论摘要\n2026-07-01,3,1000,600,42,120,9.5,旧格式",
      () => `legacy-${++sequence}`,
    )[0];
    ...
    const expanded = parseMetricsCsv(
      "date,chapter,exposure,clicks,reads,firstChapterCompletion,threeChapterRetention,retention,follows,bookshelfAdds,revenue\n2026-07-02,4,2000,900,700,48%,31%,40,140,80,15",
      () => `new-${++sequence}`,
    )[0];
    expect(expanded).toMatchObject({
      clicks: 900,
      firstChapterCompletion: 48,
      threeChapterRetention: 31,
      bookshelfAdds: 80,
    });
  });
    ...
    const metrics = [
      {
        ...
        clicks: 200,
        reads: 100,
        firstChapterCompletion: 30,
        threeChapterRetention: 20,
        retention: 55,
        follows: 10,
        bookshelfAdds: 2,
        revenue: 0,
      ...
      },
      {
        ...
        clicks: 180,
        reads: 90,
        firstChapterCompletion: 32,
        threeChapterRetention: 22,
        retention: 50,
        follows: 9,
        bookshelfAdds: 3,
        revenue: 0,
      ...
      },
      {
        ...
        clicks: 150,
        reads: 70,
        firstChapterCompletion: 25,
        threeChapterRetention: 18,
        retention: 28,
        follows: 4,
        bookshelfAdds: 1,
        revenue: 0,
...
describe("ranking schedules", () => {
  const schedule = {
    ...
    listName: "榜单",
    frequency: "每日" as const,
    enabled: true,
    lastRunAt: null,
    nextRunAt: "2026-07-30T00:00:00.000Z",
    lastStatus: "未运行" as const,
    lastError: null,
  };
  ...
  it("advances successful and failed runs to the next cycle", () => {
    ...
    expect(completeRankingSchedule(schedule, snapshot, from)).toMatchObject({
      lastStatus: "成功",
      lastRunAt: snapshot.capturedAt,
      nextRunAt: "2026-07-31T08:00:00.000Z",
    });
    expect(failRankingSchedule(schedule, new Error("访问受限"), from)).toMatchObject({
      lastStatus: "失败",
      lastError: "访问受限",
      nextRunAt: "2026-07-31T08:00:00.000Z",
    });
