...
describe("provider compatibility", () => {
  it("normalizes token usage and detects JSON mode rejection", () => {
    expect(parseProviderUsage({ usage: { prompt_tokens: 120, completion_tokens: 80 } })).toEqual({
      inputTokens: 120,
      outputTokens: 80,
    });
    expect(parseProviderUsage({ usage: { input_tokens: 12, output_tokens: 8 } })).toEqual({
      inputTokens: 12,
      outputTokens: 8,
    });
...
describe("prompt regression evaluation", () => {
  ...
  it("detects structure and fact regressions deterministically", () => {
    ...
    const bad = evaluatePromptOutput(
      fixture,
      { content: "主角已经知道密码。重复句子重复句子重复句子重复句子" },
      100,
      50,
    );
  ...
  it("keeps the current real-task corpus above its versioned baseline", () => {
    ...
    const scores = PROMPT_REGRESSION_BASELINE.fixtures.map((item) =>
      evaluatePromptOutput(item.fixture, item.output, item.inputTokens, item.outputTokens),
    );
