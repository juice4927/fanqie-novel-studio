...
import { compareQualityBenchmarkVersions, evaluateQualityBenchmark, summarizeQualityBenchmark } from "../src/shared/quality-benchmark";
...
describe("quality benchmark", () => {
  it("keeps the checked-in baseline above all quality gates", () => {
    ...
    const scores = QUALITY_BENCHMARK.cases.map(({ fixture, baselineOutput }) => evaluateQualityBenchmark(fixture, baselineOutput));
  ...
  it("fails unsupported evidence and clean-case false positives", () => {
    ...
    const unsupported = evaluateQualityBenchmark(conflict, { issues: [{ severity: "硬性", category: "资源一致性", message: "灵石不足", evidence: "账本里从未出现的证据" }] });
    ...
    const falsePositive = evaluateQualityBenchmark(clean, { issues: [{ severity: "硬性", category: "知识边界", message: "知识边界冲突", evidence: "第一张正式订单" }] });
  ...
  it("reports why a candidate prompt version regressed", () => {
    const baselineScores = QUALITY_BENCHMARK.cases.map(({ fixture, baselineOutput }) => evaluateQualityBenchmark(fixture, baselineOutput));
    const candidateScores = QUALITY_BENCHMARK.cases.map(({ fixture, baselineOutput }, index) => evaluateQualityBenchmark(fixture, index === 0 ? { issues: [] } : baselineOutput));
    const comparison = compareQualityBenchmarkVersions(summarizeQualityBenchmark(baselineScores), summarizeQualityBenchmark(candidateScores));
