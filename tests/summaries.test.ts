...
describe("structured story summaries", () => {
  it("keeps outcomes, state changes, open loops and ending momentum", () => {
    const result = buildChapterSummary(
      chapter("林舟决定潜入封锁区。他获得了旧门禁卡，却发现维修员留下的线索。真正的开门密码仍未找到，巡逻队即将抵达！"),
    );
  ...
  it("keeps book, current volume and recent stages in a bounded long-term memory", () => {
    ...
    const summaries = [
      ...
      ...Array.from({ length: 50 }, (_, index) =>
        summary(`stage-${index}`, "十章阶段", index * 10 + 1, index * 10 + 10, `阶段${index}：${"变化".repeat(400)}`),
      ),
    ];
  ...
  it("prepares versioned layered summaries without mutating project state", () => {
    ...
    const updates = prepareFinalizedChapterSummaries(project, current, "2026-07-31T00:00:00.000Z");

    expect(updates.map((item) => item.layer)).toEqual(["场景", "场景", "章节", "十章阶段", "分卷", "全书"]);
