...
const chapter: Chapter = {
  id: "chapter-1", number: 1, title: "起点", outline: "主角进入车站", content: "他推开门。灯忽然亮了。",
  wordCount: 12, status: "草稿", batchMode: "逐章", isKeyChapter: false, revision: 3, updatedAt: "2026-01-01T00:00:00.000Z",
};

describe("novel revision safeguards", () => {
  it("applies array contract fields as trimmed lines", () => {
    const contract = {
      worldRules: ["旧规则"], version: 2, approved: true,
    } as StoryContract;
    const repair = {
      id: "contract:worldRules", field: "worldRules", label: "世界规则", before: "旧规则",
      after: "新规则一\n\n 新规则二 ", reason: "同步正文", risk: "高",
  ...
  it("rejects a contract proposal when its source value changed", () => {
    ...
    const repair = {
      id: "contract:premise", field: "premise", label: "故事前提", before: "原前提",
      after: "新前提", reason: "同步正文", risk: "高",
    } satisfies NovelContractRepair;
