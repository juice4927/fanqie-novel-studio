...
function input(patch: Partial<ContextCompilerInput> = {}): ContextCompilerInput {
  ...
  return {
    ...
    summaries: [{
      id: "summary-9", layer: "章节", title: "第9章摘要", fromChapter: 9,
      toChapter: 9, content: "摘要优先内容", version: 1, updatedAt: timestamp,
    }],
    expectations: [
      { id: "linked", title: "关联期待", description: "", sourceChapter: 2, expectedPayoffChapter: 10, actualPayoffChapter: null, status: "待兑现", payoffResult: "", createdAt: timestamp, updatedAt: timestamp },
      ...Array.from({ length: 25 }, (_, index) => ({ id: `open-${index}`, title: `开放期待${index}`, description: "", sourceChapter: index + 1, expectedPayoffChapter: 20, actualPayoffChapter: null, status: "待兑现" as const, payoffResult: "", createdAt: timestamp, updatedAt: timestamp })),
    ],
