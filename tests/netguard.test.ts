...
describe("network guard", () => {
  ...
  it("rejects credentials and literal private destinations before fetch", () => {
    expect(() => assertPublicHttpUrlSyntax(new URL("https://user:secret@example.com/v1"))).toThrow("不含凭据");
    expect(() => assertPublicHttpUrlSyntax(new URL("https://[::ffff:10.0.0.1]/v1"))).toThrow("本机");
    expect(() => assertPublicHttpUrlSyntax(new URL("https://localhost/v1"))).toThrow("本机");
  });
  ...
  it("allows same-origin model redirects but rejects cross-origin redirects", async () => {
    ...
    await expect(
      fetchPublicHttpResponse("https://provider.example/v1", { method: "POST" }, { fetchImpl }),
    ...
    await expect(
      fetchPublicHttpResponse("https://provider.example/v1", { method: "POST" }, { fetchImpl: crossOrigin }),
    ).rejects.toThrow("跨来源");
  });
  ...
  it("rejects a redirect to a literal private address", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(new Response(null, { status: 307, headers: { location: "https://127.0.0.1/admin" } }));
    await expect(
      fetchPublicHttpResponse(
        "https://provider.example/v1",
        { method: "POST" },
        { fetchImpl, allowCrossOriginRedirect: true },
      ),
    ).rejects.toThrow("本机");
  });
});
