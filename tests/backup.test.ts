...
import { autoBackupFileName, createEncryptedBackup, nextAutoBackupAt, pruneAutoBackups, restoreEncryptedBackup, verifyEncryptedBackup } from "../electron/backup";
...
describe("encrypted backup", () => {
  ...
  it("rotates only managed automatic backups", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "novel-backup-rotation-")); roots.push(root);
    const managed = [
