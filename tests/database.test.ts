...
function createChapter(number: number, content = "这是一段用于验证章节版本和项目隔离的正文内容。".repeat(40)): Chapter {
  return {
    ...
    number,
    title: `第${number}章`,
    outline: "目标：验证状态；冲突：测试门禁；结果：保留证据。",
    content,
    wordCount: 0,
    status: "待质检",
    batchMode: "逐章",
...
describe("per-book isolation and gates", () => {
  ...
  it("stores bounded automatic backup scheduling metadata without a password", () => {
    ...
    const saved = database.saveAutoBackupSettings(
      { enabled: true, frequency: "weekly", retentionCount: 12 },
      nextRunAt,
    );
    ...
    expect(() =>
      database.saveAutoBackupSettings({ enabled: true, frequency: "daily", retentionCount: 31 }, nextRunAt),
    ).toThrow("1–30");
  ...
  it("records AI usage and marks abandoned running jobs as interrupted", () => {
    ...
    database.finishAiJob(id, "{}", undefined, {
      ...
      actualCost: 0.012,
      durationMs: 345,
      headersAt: "2026-07-30T10:00:01.000Z",
      firstTokenAt: "2026-07-30T10:00:02.000Z",
    ...
    expect(reopened.listAiJobs().find((job) => job.id === abandoned)).toMatchObject({
      status: "已中断",
      error: expect.stringContaining("重新执行"),
    });
  });

  it("preserves every AI attempt while caching the latest success", () => {
    const database = createDatabase();
    const first = database.startAiJob(
      null,
      "draft-chapter",
      "same-hash",
      "prompt",
    ...
    const second = database.startAiJob(
      null,
      "draft-chapter",
      "same-hash",
      "prompt",
  ...
  it("gets an AI job by id even when it falls outside the bounded history list", () => {
    const database = createDatabase();
    const target = database.startAiJob(
      "project",
      "draft-chapter",
      "target-hash",
      "prompt",
  ...
  it("does not let an AI result overwrite an autosaved chapter change", () => {
    ...
    const autosaved = database.saveChapter(
      project.id,
      {
        ...original,
  ...
  it("detects and rebuilds incomplete chapter search indexes", () => {
    ...
    expect(broken.checks).toContainEqual(
      expect.objectContaining({ id: `project-search-${project.id}`, status: "警告", repairable: true }),
    );
    const repaired = database.rebuildSearchIndexes(project.id);
    expect(repaired.checks).toContainEqual(
      expect.objectContaining({ id: `project-search-${project.id}`, status: "正常", repairable: false }),
    );
  ...
  it("reports orphan project directories without deleting them", () => {
    ...
    expect(report.checks).toContainEqual(
      expect.objectContaining({ id: "orphan-orphan-project", status: "警告", repairable: false }),
    );
  ...
  it("persists chapter intent and synchronizes the cross-chapter expectation ledger", () => {
    ...
    const chapter = database.saveChapter(project.id, {
      ...createChapter(1),
      chapterPromise: "验证能力首次反馈",
      expectedPayoff: "救人成功但付出记忆代价",
      crisis: "七分钟内必须找到伤者",
      endingExpectation: "谁在制造事故",
      expectationTargetChapter: 6,
    });
    ...
    const fulfilled = database.saveExpectation(project.id, {
      ...detail.expectations[0],
      status: "已兑现",
      actualPayoffChapter: 5,
      payoffResult: "锁定第一名交易者",
    });
  ...
  it("keeps autosaves out of revision history until a version is established", () => {
    ...
    const secondAutosave = database.saveChapter(
      project.id,
      { ...firstAutosave, content: "自动保存正文二" },
      "autosave",
    );
  ...
  it("loads project chapter metadata separately from chapter bodies", () => {
    ...
    expect(chapterColumns.map((column) => column.name)).toEqual(
      expect.arrayContaining(["number", "status", "word_count", "updated_at"]),
    );
    expect(
      (
        inspection.prepare("SELECT COUNT(*) AS count FROM records WHERE collection = 'chapters'").get() as {
          count: number;
        }
      ).count,
    ).toBe(0);
    expect(
      (
        inspection.prepare("SELECT content FROM chapter_contents WHERE chapter_id = ?").get(saved.id) as {
          content: string;
        }
      ).content,
    ).toBe(body);
  ...
  it("builds finalized summaries without loading every chapter body", () => {
    ...
    expect(() => database.transitionChapter(project.id, chapter.id, "已定稿")).not.toThrow();
    expect(fullProjectRead).not.toHaveBeenCalled();
    fullProjectRead.mockRestore();
    expect(database.getProjectOverview(project.id).summaries.map((item) => item.layer)).toEqual(
      expect.arrayContaining(["场景", "章节", "十章阶段", "全书"]),
    );
  });
  ...
  it("migrates legacy chapter records into indexed metadata and separate content", () => {
    ...
    const reopened = new WorkspaceDatabase(root);
    databases.push(reopened);
    expect(reopened.getProjectOverview(project.id).chapters.find((item) => item.id === chapter.id)).toMatchObject({
      id: chapter.id,
      number: 3,
      content: "",
    });
    ...
    expect(
      (
        migrated.prepare("SELECT COUNT(*) AS count FROM records WHERE collection = 'chapters'").get() as {
          count: number;
        }
      ).count,
    ).toBe(0);
    expect(
      (migrated.prepare("PRAGMA table_info(embeddings)").all() as Array<{ name: string }>).map((column) => column.name),
  ...
  it("loads visible dashboard activity through an exclusive cutoff without chapter bodies", () => {
    ...
    const secondVisible = database.createProject({
      title: "第二本可见作品",
      genre: "都市脑洞",
      targetWords: 1000000,
      updateCadence: "每日1章",
    ...
    const archived = database.createProject({
      title: "归档总览",
      genre: "都市脑洞",
      targetWords: 1000000,
      updateCadence: "每日1章",
    ...
    database.saveSchedule(archived.id, {
      ...
      chapterNumber: archivedChapter.number,
      chapterTitle: archivedChapter.title,
    ...
    expect(() => database.getDashboardActivity("invalid")).toThrow("仪表盘截止时间无效");
  });

  it("keeps same-name entities inside their own project databases", () => {
    const database = createDatabase();
    const one = database.createProject({
      title: "项目一",
      genre: "都市脑洞",
      targetWords: 3000000,
      updateCadence: "每日2章",
    });
    const two = database.createProject({
      title: "项目二",
      genre: "现言甜宠",
      targetWords: 3000000,
      updateCadence: "每日1章",
  ...
  it("blocks approval order and unresolved hard issues", () => {
    ...
    expect(database.getProject(project.id).summaries.map((item) => item.layer)).toEqual(
      expect.arrayContaining(["场景", "章节", "十章阶段", "全书"]),
    );
    expect(database.getProject(project.id).summaries.find((item) => item.layer === "章节")?.content).toContain(
      "未解决事项：",
    );
  ...
  it("marks overlapping facts as conflicts and ranks relevant facts", () => {
    ...
    expect(database.searchRelevantFacts(project.id, "北仓控制方", 3, 2).some((fact) => fact.subject === "北仓")).toBe(
      true,
    );
  });
  ...
  it("rebuilds fact vectors when the embedding provider changes", () => {
    ...
    const rows = inspection
      .prepare("SELECT provider_id, dimensions FROM embeddings WHERE source_type = 'facts'")
      .all() as Array<{ provider_id: string; dimensions: number }>;
  ...
  it("atomically ends the previous state when a replacement candidate is confirmed", () => {
    ...
    const previous = database.saveFact(project.id, {
      id: "",
      kind: "地点",
      subject: "林舟",
      predicate: "所在地",
    ...
    const candidate = database.saveFact(project.id, {
      id: "",
      kind: "地点",
      subject: "林舟",
      predicate: "所在地",
  ...
  it("keeps ignored candidates out of conflicts without changing active state", () => {
    ...
    const current = database.saveFact(project.id, {
      id: "",
      kind: "人物",
      subject: "林舟",
      predicate: "身份",
    ...
    const ignored = database.saveFact(project.id, {
      id: "",
      kind: "人物",
      subject: "林舟",
      predicate: "身份",
  ...
  it("versions a secret when its knowledge scope expands", () => {
    ...
    const privateFact = database.saveFact(project.id, {
      id: "",
      kind: "秘密",
      subject: "仓库密钥",
      predicate: "真实位置",
    ...
    const sharedFact = database.saveFact(project.id, {
      id: "",
      kind: "秘密",
      subject: "仓库密钥",
      predicate: "真实位置",
  ...
  it("persists review experiments without applying story changes", () => {
    ...
    expect(database.getProject(project.id).experiments[0]).toMatchObject({
      id: saved.id,
      status: "观察中",
      fromChapter: 10,
      toChapter: 15,
    ...
    expect(() =>
      database.saveReviewExperiment(project.id, { ...saved, baselineStart: "2026-07-08", baselineEnd: "2026-07-01" }),
    ).toThrow("观察日期范围无效");
    expect(() =>
      database.saveReviewExperiment(project.id, { ...saved, status: "已结论", conclusion: "", decision: null }),
    ).toThrow("结论和处理决定");
  });
});
...
describe("research firewall", () => {
  it("detects continuous-expression reuse without copying source into projects", () => {
    ...
    database.saveResearchBook(
      ...
      [{ title: "第一章", content: unique.repeat(2), wordCount: unique.length * 2 }],
    );
...
describe("approval gates", () => {
  it("moves chapter state and publication schedule in one database operation", () => {
    ...
    expect(database.getProjectSummary(project.id)).toMatchObject({
      ...
      chapterCount: 1,
      stockChapters: 1,
      nextPublishAt: "2026-08-01T08:00:00.000Z",
    });
  ...
  it("persists each project's aesthetic profile in the story contract", () => {
    ...
    const saved = database.saveContract(project.id, {
      ...
      aestheticProfile: {
        ...
        proseTexture: "明快",
        dialogueStyle: "短促",
        emotionalExpression: "直接",
        signatureTechniques: ["多人交锋"],
        avoidPatterns: ["旁白说教"],
      },
    });
  ...
  it("requires an approved change request before editing an approved contract", () => {
    ...
    const initial = database.saveContract(project.id, {
      ...database.getProject(project.id).contract,
      premise: "主角发现异常规则",
      readerPromise: "持续解谜与成长",
      ending: "主角关闭规则源头",
      ...completeNarrativeEngine,
    });
  ...
  it("owns lifecycle status and binds approvals to one target version", () => {
    ...
    const contract = database.saveContract(project.id, {
      ...database.getProject(project.id).contract,
      premise: "前提",
      readerPromise: "承诺",
      ending: "终局",
  ...
  it("invalidates a pending schedule when an approved protected edit returns a chapter to quality review", () => {
    ...
    const saved = database.saveChapter(project.id, {
      ...database.getChapter(project.id, chapter.id),
      content: "修订正文",
    });
    const detail = database.getProject(project.id);
  ...
  it("binds planning changes to the current persisted plan version", () => {
    ...
    expect(() => database.saveChangeRequest(project.id, { ...request, targetId: "missing" })).toThrow(
      "变更目标规划不存在",
    );
